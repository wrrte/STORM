...
def joint_train_world_model_agent(env_name, max_steps, num_envs, image_size,
    ...
    for total_steps in tqdm(range(max_steps//num_envs)):
        ...
        # SMC(Selective Memory Consolidation) - Surprise 계산
        if replay_buffer.ready() and len(context_action) > 1:
            world_model.eval()
            with torch.no_grad():
                # 새로 받은 obs를 모델 입력 형태로 변환
                obs_tensor = rearrange(torch.Tensor(obs).to(device), "B H W C -> B 1 C H W") / 255.0
                # 모델이 이전 step에서 예측한 잠재 상태(prior_flattened_sample)를 이미지로 디코딩
                obs_hat = world_model.image_decoder(prior_flattened_sample)
